...
import { useNavigate } from "react-router-dom"; // 👈 ഇതാണ് നാവിഗേഷന് വേണ്ടി ആഡ് ചെയ്തത്
...
const SearchBar = () => {
  ...
  const navigate = useNavigate(); // 👈 നാവിഗേറ്റ് ചെയ്യാൻ

  const handleSearch = (e) => {
    e.preventDefault();

    // URL-ലേക്ക് പാസ്സ് ചെയ്യാനുള്ള ഡാറ്റ സെറ്റ് ചെയ്യുന്നു
    const queryParams = new URLSearchParams();

    if (location.trim()) queryParams.append("location", location.trim());
    if (propertyType) queryParams.append("type", propertyType);
    if (priceRange.trim()) queryParams.append("budget", priceRange.trim());

    // Properties പേജിലേക്ക് ഈ ഡാറ്റ വെച്ച് നാവിഗേറ്റ് ചെയ്യുന്നു
    navigate(`/properties?${queryParams.toString()}`);
  };

  // 🎨 Ultra-Modern High-Fidelity Design System
  const styles = {
    searchBarContainer: {
      width: "100%",
      maxWidth: "950px",
    ...
    btnSearch: {
      ...
      fontWeight: "700",
      cursor: "pointer",
      boxShadow: isBtnHovered ? "0 12px 20px -6px rgba(5, 150, 105, 0.4)" : "0 8px 16px -4px rgba(16, 185, 129, 0.3)",
